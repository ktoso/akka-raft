...
trait InternalProtocol {
  ...
  case class AppendRejected(follower: ActorRef, term: Term)    extends FollowerResponse
  case class AppendSuccessful(follower: ActorRef, term: Term)  extends FollowerResponse
